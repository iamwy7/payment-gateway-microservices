...
func (r *InvoiceRepository) UpdateStatus(invoice *domain.Invoice) error {
	// Pessimist Lock using SELECT FOR UPDATE to previne race conditions on invoice status post processing
	tx, err := r.db.Begin()
	if err != nil {
		return err
	}
	...
	var currentStatus domain.Status // for logs
	err = tx.QueryRow(`SELECT status FROM invoices WHERE id = $1 FOR UPDATE`, invoice.ID).Scan(&currentStatus)
